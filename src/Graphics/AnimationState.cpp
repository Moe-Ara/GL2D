...
namespace Graphics {
    ...
    AnimationState::AnimationState(std::shared_ptr<Animation> animation, std::shared_ptr<Managers::Animator> animator)
    ...
    void
    AnimationState::addTransition(const std::shared_ptr<Utils::IState> &targetState, std::function<bool()> condition) {
        m_transitions.emplace_back(targetState, std::move(condition));
    }

    const std::vector<std::pair<std::weak_ptr<Utils::IState>, std::function<bool()>>> &
