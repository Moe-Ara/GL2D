...
public:
  AnimationState(std::shared_ptr<Animation> animation,
  ...
  void addTransition(const std::shared_ptr<Utils::IState> &targetState,
...
private:
  ...
  std::vector<std::pair<std::weak_ptr<IState>, std::function<bool()>>>
      m_transitions;
};
