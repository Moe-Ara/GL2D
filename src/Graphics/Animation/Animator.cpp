...
namespace Managers {
    ...
    void Animator::update(float deltaTime) {
        ...
        float frameDuration = resolveFrameDuration(m_currentAnimation->getFrame(m_currentFrameIndex));
        while (m_elapsedTime >= frameDuration && m_isPlaying) {
            ...
            frameDuration = resolveFrameDuration(m_currentAnimation->getFrame(m_currentFrameIndex));
    ...
    void Animator::setAnimationFinishedCallback(AnimationFinishedCallback callback) {
    ...
    void Animator::updateSpriteUV() {
        ...
        auto texture = frame.texture ? frame.texture : m_currentAnimation->getSharedTexture();
        m_sprite->setTexture(texture);

        float frameWidth = 1.0f / static_cast<float>(std::max(1, m_currentAnimation->getTotalCols()));
        float frameHeight = 1.0f / static_cast<float>(std::max(1, m_currentAnimation->getTotalRows()));

        glm::vec4 uv = frame.useCustomUV
                       ? frame.uvRect
                       : glm::vec4(
    ...
    void Animator::dispatchFrameChanged(const Graphics::Frame &frame) {
        ...
        if (m_frameEventCallback && m_currentAnimation && !frame.eventName.empty()) {
            m_frameEventCallback(frame.eventName, m_currentAnimation, m_currentFrameIndex);
        }
    }

    void Animator::dispatchAnimationFinished() {
        if (m_hasNotifiedCompletion || !m_animationFinishedCallback || !m_currentAnimation) {
            return;
        }
    ...
    float Animator::resolveFrameDuration(const Graphics::Frame &frame) const {
        float duration = frame.duration > 0.0f ? frame.duration :
                         (m_currentAnimation ? m_currentAnimation->getFrameDuration() : 0.016f);
        if (duration <= 0.0f) {
            duration = 0.016f;
        }
        return duration;
